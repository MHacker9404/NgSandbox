...
export class CustomerDetailComponent implements OnInit {
    ...
    _isEditMode: boolean;
    @ViewChild('myForm', { static: true }) _myForm: NgForm;

    constructor(private _router: Router, private _route: ActivatedRoute, private _zzaRepository: ZzaDataService) {}
    ...
    async ngOnInit() {
        try {
            if (this._route.routeConfig.path !== 'customer-add') {
                ...
                // add customer
                this._customer = <Customer>await this._zzaRepository.createEntity('Customer');
            }
        } catch (error) {
    ...
    getValidationErrors(propertyName: string) {
        ...
        const errors = this._customer.entityAspect.getValidationErrors(propertyName);

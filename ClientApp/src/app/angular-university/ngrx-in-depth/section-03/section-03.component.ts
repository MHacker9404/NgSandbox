...
import * as fromSection03 from './state/section03.reducer';

@Component({
    selector: 'ngs-section-03',
    template: `
...
export class Section03Component implements OnInit {
    ...
    ngOnInit() {
        this.isLoggedIn$ = this._state$.pipe(
            ...
            select(isLoggedIn),
            tag('section03:isLoggedIn')
        );

        this.isLoggedOut$ = this._state$.pipe(
            // map((state: any) => !state.auth.isLoggedIn),
            select(isLoggedOut),
            tag('section03:isLoggedOut')
        );
    }
...
export class Section03RoutingModule {}
...
@NgModule({
    ...
    imports: [
        ...
        MatToolbarModule,
        SharedModule,
        StoreModule.forFeature(fromSection03.section03FeatureKey, fromSection03.reducer),
        Section03RoutingModule,
        AuthModule.forRoot(),
        CoursesModule,
    ],
})
